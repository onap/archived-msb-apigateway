...
routeUtil.groupRouteByPort=function(resp){
   var routeArray=new Array();
   var routeGroupArray=[[]];
    for(var i=0;i<resp.length;i++){ 

          var publish_port=resp[i].publish_port;
          

            //Loop all packet classification
            if(routeArray[publish_port] == undefined){  
                var list = [];  
                list.push(resp[i]);  
                routeArray[publish_port] = list;  
            }else{  
                routeArray[publish_port].push(resp[i]);  
            }  
        ...
        //Sorting through all quantity > 1
            ...
            for(var groupServiceName in routeArray){  
              ...
              if(groupServiceName==""){
                routeGroupArray[0]=routeArray[groupServiceName];
              }
              else{
                 routeGroupArray.push(routeArray[groupServiceName]);
              }
               
            }


             for(var i=0;i<routeGroupArray.length;i++){
...
routeUtil.showGroupPort=function(index,serviceArray){
   
   var defaultPort=vm.route.routePort+" <span class='protocal'>http</span> / 443 <span class='protocal'>https</span>";

   if(serviceArray[0]==null) {return defaultPort;}

  var publish_port=serviceArray[0].publish_port

   if(publish_port==""){ return defaultPort;}
   else { return publish_port}

}

routeUtil.showPotocol=function(publish_port,publishProtocol){
   
   if(publish_port=="") return "";

   return "<div class=\"protocal\" style=\"float:right\">"+publishProtocol+"</div>";
    ...
    var routeGroupArray=[[]];
     ...
     for(var i=0;i<resp.length;i++){ 
          ...
          if(reg_match1.test(fullServiceName)){  
            ...
            groupServiceName=$.i18n.prop('org_onap_msb_route_property_root');
          }
...
routeUtil.showGroupName=function(index,serviceArray){
    ...
    if(index==maxGroupSN){
        return $.i18n.prop('org_onap_msb_route_property_other_group');
    }
    else{
...
routeUtil.showStatus=function(status){
   if(status === '1'){
      return " <span class='label label-success'>"+$.i18n.prop('org_onap_msb_route_property_normal')+"</span>";
   }
   else if(status === '0'){
      return " <span class='label label-danger'>"+$.i18n.prop('org_onap_msb_route_property_disable')+"</span>";
   }
   else {
      return " <span class='label label-info'>"+$.i18n.prop('org_onap_msb_route_property_unknown')+"</span>";
   }
...
routeUtil.showVisualRange=function(visualRange){
    ...
    for(var i=0;i<rangArray.length;i++){
        if(rangArray[i] === '0'){
              visualRangeText+= $.i18n.prop('org_onap_msb_route_form_intersystem')+"   ";
           }
         else if(rangArray[i] === '1'){
              visualRangeText+= $.i18n.prop('org_onap_msb_route_form_insystem')+"   ";
           }
...
var serviceName=vm.msbRouteInfo.serviceName==""?"serviceName":vm.msbRouteInfo.serviceName;
  ...
  if(vm.msbRouteInfo.protocol=='UI'){
   ...
   else if(vm.msbRouteInfo.protocol=='HTTP'){
